...
def main():
    with open(f'../splits/{dataset}.txt') as val_file:
...
def compute_averages(aps):
    ...
    avg_dict['all_ap_50%'] = np.nanmean(aps[d_inf, :, o50])  # 所有>0.5的
    avg_dict['all_ap_25%'] = np.nanmean(aps[d_inf, :, o25])  # 所有>0.25的
    avg_dict["classes"] = {}
    for (li, label_name) in enumerate(CLASS_LABELS):
...
def save_points_as_ply(points, save_path):
    """
    Save 3D points to a .ply file without color.

    Args:
        points (np.ndarray): (N, 3) point coordinates
        save_path (str): output .ply file path
    """
    # Create structured array with only x, y, z
    ...
    # Write PLY
    ply = PlyElement.describe(vertex, 'vertex')
    PlyData([ply]).write(save_path)
...
def assign_instances_for_scan(pred_file, gt_file):
    ...
    for pred_mask_file in (pred_info):
        ...
        if len(pred_mask) != len(gt_ids):
            print(
                'wrong number of lines in ' + pred_mask_file + '(%d) vs #mesh vertices (%d), please double check and/or re-download the mesh' % (
        ...
        # if label_name == 'chair':
        #     data = torch.load('/home/xf/datasets/ScanNetPP/pcld_0.25/09c1414f1b.pth')
        #     points = np.asarray(data['sampled_coords'])
        #     masked_points = points[pred_mask]
        #     # Skip if no points selected
        #     if masked_points.size == 0:
        #         print("Warning: empty mask for chair instance")
        #         continue  # or skip saving
        #     # Save PLY
        #     save_path = os.path.join(f"../temp/chair_pred.ply")
        #     save_points_as_ply(masked_points, save_path)
        #
        #     for gi in range(gt_tensor.shape[1]):
        #         gt_mask = gt_tensor[:,gi].cpu().numpy()
        #         gt_mask = gt_mask.astype(bool)
        #         masked_points = points[gt_mask]
        #         if masked_points.size == 0:
        #             print("Warning: empty mask for chair instance")
        #             continue  # or skip saving
        #         save_path = os.path.join(f"../temp/chair_gt{gi}.ply")
        #         save_points_as_ply(masked_points, save_path)
...
def print_results(avgs):
    ...
    line += "{:>15.5f}".format(all_ap_avg) + sep
    line += "{:>15.5f}".format(all_ap_50o) + sep
    line += "{:>15.5f}".format(all_ap_25o) + sep
    print(line)
    print("")
