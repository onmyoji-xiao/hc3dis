...
class ScanNetDataset:
    ...
    def __init__(self, args) -> None:
        self.scene_dir = f'{args.image_root}/{args.seq_name}'
        self.rgb_dir = f'{self.scene_dir}/color'
        self.depth_dir = f'{self.scene_dir}/depth'
        self.segmentation_dir = f'{args.mask_root}/{args.seq_name}'
        self.point_cloud_path = f'{args.pc_root}/{args.seq_name}/{args.seq_name}_vh_clean_2.ply'
        self.mesh_path = self.point_cloud_path
        self.extrinsics_dir = f'{self.scene_dir}/pose'
        self.intrinsic_path = f'{self.scene_dir}/intrinsic_depth.txt'
    ...
    def get_frame_list(self, stride):
        ...
        frame_id_list = [int(x.split('.')[0]) for x in image_list]
        return list(frame_id_list)

    def get_intrinsics(self, frame_id=None):
        intrinsics = np.loadtxt(self.intrinsic_path)

        intrinisc_cam_parameters = o3d.camera.PinholeCameraIntrinsic()
        intrinisc_cam_parameters.set_intrinsics(self.image_size[0], self.image_size[1], intrinsics[0, 0],
                                                intrinsics[1, 1], intrinsics[0, 2], intrinsics[1, 2])
